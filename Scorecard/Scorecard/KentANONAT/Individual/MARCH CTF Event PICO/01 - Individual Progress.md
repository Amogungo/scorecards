# 📊 Individual Progress Scorecard

## Daily Training Log

**Date:** _______March 14 15 and 16________ | **Training Day:** 16__/180 (or _16_/90 for 3-month plan)

### 1. Time Investment

- [ ] Training time today: ____2___ hours
- [V ] Goal met? (Yes/No)
- [ ] Consistency streak: __6___ days

**Points:**

- 2+ hours = 10 points
- 1-2 hours = 5 points 5
- <1 hour = 2 points
- Missed day = 0 points (streak resets)

### 2. Challenge Completion

| Difficulty      | Challenges Solved | Points Earned |
| --------------- | ----------------- | ------------- |
| Easy            | __1___ × 5 pts     | ___5__         |
| Medium          | ____1_ × 15 pts    | __15___         |
| Hard            | _____ × 30 pts    | _____         |
| Expert          | _____ × 50 pts    | _____         |
| **Daily Total** |                   | **___20__**     |

### 3. Quality Indicators

- [ V] Created writeup for at least 1 challenge (+10 pts)
- [ ] Reviewed 3+ writeups from others (+5 pts)
- [V ] Learned new technique/tool (+10 pts)
- [ ] Updated cheat sheet (+5 pts)
- [ ] Practiced timed challenge (+5 pts)

**Quality Points Total:** _20____

### 4. Category Focus Today

Which categories did you practice?

- [ ] Web Exploitation
- [V ] Binary Exploitation / Pwn
- [ ] Cryptography
- [ ] Reverse Engineering
- [ ] Forensics
- [ ] OSINT
- [ ] Other: ___________

**Primary Specialty:** _120____ minutes

**Secondary Specialty:** _____ minutes

**Other Categories:** _____ minutes

### 5. Reflection (Qualitative)

**What went well today?**

--- Breaking the code with buffering vulnerability 

**What challenged you?**

--- timed challenge

**Key learning:**

--- Buffer vulnerability leaks code and should be avoided

**Tomorrow's focus:**

--- PicoCTF basica binary exploitation and understanding other attacks
